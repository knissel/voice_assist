...
export const TimersPanel: React.FC = () => {
    ...
    return (
        <section className={`${styles.timersPanel} ${activeTimers === 0 ? styles.isEmpty : ''}`}>
            ...
            <div className={styles.timersList}>
                {activeTimers === 0 ? (
                    ...
                    timers.map(timer => {
                        ...
                        const isComplete = remaining <= 0; // Simplified complete check
                        ...
                        return (
                            <div key={timer.id} className={`${styles.timerCard} ${isUrgent ? styles.urgent : ''} ${isComplete ? styles.complete : ''}`}>
                                ...
                                <div className={styles.timerInfo}>
                                    <div className={styles.timerName}>{timer.label || 'Timer'}</div>
                                ...
                                <button className={styles.timerCancel} onClick={() => cancelTimer(timer.label)}>x</button>
                            </div>
                        );
                    })
                )}
